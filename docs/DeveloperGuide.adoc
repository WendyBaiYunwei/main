...
.PlannerAddCommand component interactions
...
.PlannerRemoveCommand component interactions
...
is the main recommendation list that sorts the modules in a specific way and displays maximum 10 most recommended modules. The second and the third list respectively displays the modules with matching tags and modules with matching credits. The modules in the two lists should also exist in the main recommendation list.

If `tag` is supplied as a parameter in input, modules will be sorted according to tags first. Modules with greater number of tags that match the desirable tags will be prioritized. If tie, modules with credits closer to the desirable credits will be prioritized. If tie, modules will be sorted according to alphabetical order.

.PlannerSuggestCommand component interactions
image::PlannerSuggestCommandAcitivityDiagram.png[width="650"]
